...
describe('when generating xsd for domain entity', (): void => {
  ...
  it('should have an Elements sheet with the correct rows', (): void => {
    ...
    expect(workbook.sheets[0].rows[5].values.reduce(rowToString)).toMatchInlineSnapshot(
    ...
    expect(workbook.sheets[0].rows[6].values.reduce(rowToString)).toMatchInlineSnapshot(
      `"EffectiveEndDate, xs:date, DescriptorType, minOccurs: 0, The end date of the period when the descriptor is in effect."`,
    );
    expect(workbook.sheets[0].rows[7].values.reduce(rowToString)).toMatchInlineSnapshot(
      `"MyAddressLine, MyAddressLine, DomainEntityName, , An address line."`,
    );
    expect(workbook.sheets[0].rows[8].values.reduce(rowToString)).toMatchInlineSnapshot(
    ...
    expect(workbook.sheets[0].rows[9].values.reduce(rowToString)).toMatchInlineSnapshot(
      `"Namespace, URI, DescriptorType, , A globally unique identifier for this descriptor."`,
    );
    expect(workbook.sheets[0].rows[10].values.reduce(rowToString)).toMatchInlineSnapshot(
    ...
    expect(workbook.sheets[0].rows[11].values.reduce(rowToString)).toMatchInlineSnapshot(
      `"ShortDescription, ShortDescription, DescriptorType, , A shortened description for the descriptor."`,
    );
    expect(workbook.sheets[0].rows[12].values.reduce(rowToString)).toMatchInlineSnapshot(
      `"TotalInstructionalDays, xs:int, DomainEntityName, , The total instructional days during the academic week."`,
    );
    expect(workbook.sheets[0].rows[13].values.reduce(rowToString)).toMatchInlineSnapshot(
  ...
  it('should have a complex types sheet with the correct rows', (): void => {
    ...
    expect(workbook.sheets[1].rows[0].values.reduce(rowToString)).toMatchInlineSnapshot(
      `"ComplexObjectType, This is the base type from which all entity elements are extended."`,
    );
    expect(workbook.sheets[1].rows[1].values.reduce(rowToString)).toMatchInlineSnapshot(
    ...
    expect(workbook.sheets[1].rows[2].values.reduce(rowToString)).toMatchInlineSnapshot(
      `"DescriptorType, This is the base for the Descriptor type."`,
    );
    expect(workbook.sheets[1].rows[3].values.reduce(rowToString)).toMatchInlineSnapshot(
      `"DomainEntityName, Domain entity documentation"`,
    );
    expect(workbook.sheets[1].rows[4].values.reduce(rowToString)).toMatchInlineSnapshot(
      `"DomainEntityNameIdentityType, Identity of a DomainEntityName."`,
    );
    expect(workbook.sheets[1].rows[5].values.reduce(rowToString)).toMatchInlineSnapshot(
    ...
    expect(workbook.sheets[1].rows[6].values.reduce(rowToString)).toMatchInlineSnapshot(
  ...
  it('should have a simple types sheet with the correct rows', (): void => {
    ...
    expect(workbook.sheets[2].rows[0].values.reduce(rowToString)).toMatchInlineSnapshot(
      `
      "CodeValue, minLength: 1
      maxLength: 50, A code or abbreviation for an element."
    `,
    );
    expect(workbook.sheets[2].rows[1].values.reduce(rowToString)).toMatchInlineSnapshot(
      `"Currency, , U.S. currency in dollars and cents."`,
    );
    expect(workbook.sheets[2].rows[2].values.reduce(rowToString)).toMatchInlineSnapshot(
      `
    ...
    expect(workbook.sheets[2].rows[3].values.reduce(rowToString)).toMatchInlineSnapshot(
      `
      "MyAddressLine, minLength: 100
      maxLength: 0, An address line."
    `,
    );
    expect(workbook.sheets[2].rows[4].values.reduce(rowToString)).toMatchInlineSnapshot(
      `
      "Percent, minValue: 0
      maxValue: 1
      totalDigits: 5
    ...
    expect(workbook.sheets[2].rows[5].values.reduce(rowToString)).toMatchInlineSnapshot(
