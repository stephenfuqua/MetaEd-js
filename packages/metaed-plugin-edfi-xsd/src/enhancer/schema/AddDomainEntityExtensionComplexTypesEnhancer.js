...
export function enhance(metaEd: MetaEdEnvironment): EnhancerResult {
  metaEd.entity.domainEntityExtension.forEach(domainEntityExtension => {
    if (domainEntityExtension.data.edfiXsd.xsd_HasExtensionOverrideProperties()) {
      ...
      domainEntityExtensionEdfiXsd.xsd_ComplexTypes = [
        createCoreRestrictionForExtensionParent(domainEntityExtension),
      ];
      domainEntityExtensionEdfiXsd.xsd_ComplexTypes.push(...createDefaultComplexType(domainEntityExtension, typeGroupDomainEntity, restrictionName(domainEntityExtension)));
    } else {
      if (domainEntityExtension.baseEntity == null) return;
      domainEntityExtension.data.edfiXsd.xsd_ComplexTypes =
        createDefaultComplexType(domainEntityExtension, typeGroupDomainEntity, domainEntityExtension.baseEntity.data.edfiXsd.xsd_MetaEdNameWithExtension());
    }
  });
