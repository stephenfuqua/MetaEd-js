...
function calculateMinOccurs(entityProperty: EntityProperty, minOccursOverride: string = ''): string {
  if (minOccursOverride !== '') return minOccursOverride;
  return entityProperty.isOptional || entityProperty.isOptionalCollection ? '0' : '';
}

function calculateMaxOccursIsUnbounded(entityProperty: EntityProperty, maxOccursIsUnboundedOverride: boolean = false): boolean {
  if (maxOccursIsUnboundedOverride) return true;
...
export function createXsdElementFromProperty(property: EntityProperty, minOccursOverride: string = '', maxOccursIsUnboundedOverride: boolean = false): Element {
...
export function createSchemaComplexTypeItems(complexTypeItemProperties: Array<EntityProperty>, minOccursOverride: string = '', maxOccursIsUnboundedOverride: boolean = false): Array<ComplexTypeItem> {
