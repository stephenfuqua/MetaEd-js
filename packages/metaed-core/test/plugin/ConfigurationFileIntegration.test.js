...
import type { PluginManifest } from '../../src/plugin/PluginManifest';
import type { PluginEnvironment } from '../../src/plugin/PluginEnvironment';
import type { ConfigurationSchema } from '../../src/plugin/ConfigurationSchema';
...
function applyConfigFileRuleOnXsdPlugin(state: State) {
  const xsdPluginManifest: ?PluginManifest = state.pluginManifest.find(manifest => manifest.shortName === 'edfiXsd');
...
describe('when loading a project with one invalid and one valid plugin configuration file', () => {
  ...
  it('should annotate xsd plugin with config data', async () => {
    const pluginEnvironment: ?PluginEnvironment = state.metaEd.plugin.get('edfiXsd');
