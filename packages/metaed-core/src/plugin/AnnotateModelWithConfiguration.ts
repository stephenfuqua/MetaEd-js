...
function findMatchingNamespaces(
  ...
  if (matches.namespace) {
    const namespaceNamesInMatches: Array<string> = [].concat(matches.namespace as any);
...
function findMatchingEntities(
  ...
  const entityTypes: Array<ModelType> = [].concat(matches.entity as any);
  ...
  const entityNamesInMatches: Array<string> = [].concat(matches.entityName as any);
...
export function annotateModelWithConfiguration(
  ...
  configRuleArray.forEach((configRule: ConfigurationRule) => {
    if (configRule.matches == null) {
      ...
      const matchesForRule: Array<ConfigurationMatches> = [].concat(configRule.matches as any);
