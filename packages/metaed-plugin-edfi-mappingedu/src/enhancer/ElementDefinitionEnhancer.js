...
import type { ComplexType, ComplexTypeItem, SimpleType, Element, ElementGroup } from 'metaed-plugin-edfi-xsd';
...
const createElementDefinitionForDescriptorReferenceType = (
  ...
  const descriptorReference: ?ComplexType = pluginDescriptorExtendedReferencesForNamespace(metaEd, namespace).get(
    element.value.type,
  );
  if (descriptorReference != null) {
    const descriptorBaseType: ?ComplexType = baseTypeFor(metaEd, namespace, descriptorReference);
...
const createElementDefinitionsFor = (
  ...
  while (elements.length > 0) {
    const elementOrElementGroup: { value: Element | ElementGroup, entityPath: Array<string> } = elements.pop();
    if (isElementGroup(elementOrElementGroup.value)) {
      const elementGroup: { value: ElementGroup, entityPath: Array<string> } = {
        value: asElementGroup(elementOrElementGroup.value),
    ...
    const element: { value: Element, entityPath: Array<string> } = {
      value: asElement(elementOrElementGroup.value),
      entityPath: elementOrElementGroup.entityPath,
    };
    ...
    const reference: ?ComplexType | ?SimpleType = referenceFor(metaEd, namespace, element.value);
    ...
    if (reference != null) {
      ...
      if (hasItems(reference))
        collectElementsFor(asComplexType(reference), [...element.entityPath, element.value.name], elements);
    }
  }
...
const createElementDefinitionForBaseType = (
  ...
  const baseType: ?ComplexType = baseTypeFor(metaEd, namespace, complexType);
