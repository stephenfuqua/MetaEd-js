...
function buildReferencedUsageInfo(referentialProperty: ReferentialProperty, rootEntityName: string, isOptional: boolean = false): ReferenceUsageInfo {
...
export function getReferenceUsageInfoList(matchingPropertyTypes: Array<PropertyType>,
  ...
  isParentRelationshipOptional: boolean = false): Array<ReferenceUsageInfo> {
  ...
  if (matchingPropertyTypes.includes(referentialProperty.type) &&
    ...
    const referencedEntityProperties: Array<ReferentialProperty> = ((referentialProperty.referencedEntity.properties.filter(isReferentialProperty): any): Array<ReferentialProperty>);
    results.push(...referencedEntityProperties.reduce((result: Array<ReferenceUsageInfo>, referencedEntityProperty: ReferentialProperty) => {
      result.push(...getReferenceUsageInfoList(matchingPropertyTypes, entityExclusionList, previouslyMatchedProperties, referencedEntityProperty, rootEntityName, isOptional));
      return result;
    }, []));
  }
  return results;
...
export function topLevelEntitiesFrom(mergedInterchange: MergedInterchange): Array<TopLevelEntity> {
  ...
  const identityTemplatesTopLevelEntities: Array<TopLevelEntity> = mergedInterchange.identityTemplates.reduce((array, element) => {
    array.push(...getEntityAndParents(element.referencedEntity));
    return array;
  }, []);
...
export function topLevelReferencePropertiesFrom(mergedInterchange: MergedInterchange): Array<ReferentialProperty> {
  const interchangeTopLevelReferenceProperties: Array<ReferentialProperty> = mergedInterchange.elements.reduce((array, element) => {
    array.push(...getPropertiesToScan(element.referencedEntity, false));
    return array;
  }, []);

  const identityTemplatesTopLevelReferenceProperties: Array<ReferentialProperty> = mergedInterchange.identityTemplates.reduce((array, element) => {
