...
export function validate(metaEd: MetaEdEnvironment): Array<ValidationFailure> {
  ...
  getPropertiesOfType(metaEd.propertyIndex, ...validTypes).forEach(property => {
    ...
    asReferentialProperty(property).mergedProperties.forEach(mergedProperty => {
      const prefix: string = (property.withContext && (property.withContext !== property.metaEdName)) ? property.withContext : '';
      ...
      failures.push({
        ...
        category: 'error',
        message: `Merge statement must start first property path with the current property ${prefix ? 'context and ' : ''}name: ${prefix}${property.metaEdName}.`,
