...
export function validate(metaEd: MetaEdEnvironment): Array<ValidationFailure> {
  ...
  getPropertiesOfType(metaEd.propertyIndex, ...validPropertyTypes).forEach(property => {
    ...
    asReferentialProperty(property).mergedProperties.forEach(mergedProperty => {
      let entity = null;
      const mergePropertyLength = mergedProperty.targetPropertyPath.length;
      for (let i = 0; i < mergePropertyLength - 1; i += 1) {
        const entityName = mergedProperty.targetPropertyPath[i];
        entity = getEntity(metaEd.entity, entityName, ...validModelTypes);
        if (entity == null) {
          failures.push(getFailure(
            `Merge statement target property path ${mergedProperty.targetPropertyPath.join('.')} must be a valid path. No Entity ${entityName} was found`,
            mergedProperty.sourceMap.targetPropertyPath[0]));
          return;
        }
      }

      if (entity != null) {
        const propertyName = mergedProperty.targetPropertyPath[mergePropertyLength - 1];
        const foundProperty = asTopLevelEntity(entity).properties.find(prop => prop.metaEdName === propertyName);
        if (foundProperty == null) {
          failures.push(getFailure(
            `Merge statement target property path ${mergedProperty.targetPropertyPath.join('.')} must be a valid path. No Property ${propertyName} was found`,
            mergedProperty.sourceMap.targetPropertyPath[0]));
        }
      }
    });
