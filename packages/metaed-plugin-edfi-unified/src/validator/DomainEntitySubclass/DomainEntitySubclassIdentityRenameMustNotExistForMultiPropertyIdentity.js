// @flow
import type { DomainEntity, MetaEdEnvironment, ValidationFailure } from 'metaed-core';
...
export function validate(metaEd: MetaEdEnvironment): Array<ValidationFailure> {
  ...
  metaEd.entity.domainEntitySubclass.forEach(domainEntitySubclass => {
    ...
    if (
      !domainEntitySubclass.identityProperties.some(x => x.isIdentityRename) ||
      (baseEntity && baseEntity.identityProperties.length <= 1)
    )
      return;
    failures.push({
