...
describe('when generating change event scripts and comparing to ODS/API 3.1 authoritative artifacts', () => {
  ...
  const authoritativeFilename: string = 'DeleteTrackingTrigger-v3.1-Authoritative.sql';
  const generatedFilename: string = 'DeleteTrackingTrigger-v3.1.sql';
    ...
    generatedOutput = R.head(
      R.head(
        state.generatorResults.filter(
          x => x.generatorName === 'edfiOdsChangeQuery.CreateDeletedForTrackingTriggersGenerator',
        ),
      ).generatedOutput,
    );
...
describe('when generating change event scripts with simple extensions and comparing to ODS/API 3.1 authoritative artifacts', () => {
  ...
  const authoritativeCoreFilename: string = 'DeleteTrackingTrigger-v3.1-Authoritative.sql';
  const authoritativeExtensionFilename: string = 'sample-DeleteTrackingTrigger-v3.1-Authoritative.sql';
  const generatedCoreFilename: string = 'DeleteTrackingTrigger-v3.1.sql';
  const generatedExtensionFilename: string = 'sample-DeleteTrackingTrigger-v3.1.sql';
  ...
  beforeAll(async () => {
    ...
    const generatorResult: GeneratorResult = R.head(
      state.generatorResults.filter(x => x.generatorName === 'edfiOdsChangeQuery.CreateDeletedForTrackingTriggersGenerator'),
    );
