...
describe('when TemplateSpecificTablePropertyEnhancer enhances table with foreign keys', (): void => {
  ...
  it('should have correct foreign key order', (): void => {
    const { foreignKeys }: any = tableEntities(metaEd, namespace).get(tableName) as Table;
  ...
  it('should have correct foreign key column order', (): void => {
    const { parentTableColumnNames }: any = R.last((tableEntities(metaEd, namespace).get(tableName) as Table).foreignKeys);
    ...
    const { foreignTableColumnNames }: any = R.last((tableEntities(metaEd, namespace).get(tableName) as Table).foreignKeys);
