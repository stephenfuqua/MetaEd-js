...
const existsSelect: string = 'SELECT COUNT(1) ';
const isUniqueSelect: string = 'SELECT is_unique ';
...
function whereClause(index: DatabaseIndex): string {
  const clause: Array<string> = index.columns.map((column: string, x: number) =>
