...
describe('when generating change event schema and comparing to ODS/API 5.4 authoritative artifacts', (): void => {
  const artifactPath: string = path.resolve(__dirname, './artifact/schema/v5.4');
  const authoritativeFilename = '0010-CreateChangesSchema-Authoritative.sql';
  const generatedFilename = '0010-CreateChangesSchema.sql';
