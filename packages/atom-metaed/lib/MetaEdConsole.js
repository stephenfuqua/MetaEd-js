...
import { getMetaEdConsoleSourceDirectory,
...
export default class MetaEdConsole {
  ...
  _verifyGulpInputs(taskName: string, isExtensionProject: boolean = false): ?GulpInputs {
    ...
    if (!fs.existsSync(gulpFilePath)) {
      this._metaEdLog.addMessage(`Unable to find gulpfile.js file in MetaEd Console Source Directory at configured path "${metaEdConsoleSourceDirectory}".`);
      this._metaEdLog.addMessage('Please configure MetaEd Console Source Directory under the atom-metaed package to target the prebuilt MetaEd Console root directory.');
      return null;
    }
    ...
    if (!fs.existsSync(gulpPath)) {
      ...
      if (!fs.existsSync(gulpPath)) {
        this._metaEdLog.addMessage(`Unable to find the gulp.cmd executable in the Core MetaEd Source Directory at configured path "${coreMetaEdSourceDirectory}" or its parent.`);
        return null;
      }
    }
  ...
  _createGulpTaskParams(gulpInputs: GulpInputs) {
    const params = [
      ...
      '--artifactPath', gulpInputs.artifactPath,
    ];
  ...
  _executeGulpTask(gulpInputs: GulpInputs) {
    ...
    const childProcess = this._spawn(
      gulpInputs.cmdFullPath,
      gulpTaskParams, { cwd: gulpInputs.metaEdConsoleSourceDirectory });
